...
test("imps init creates a project-local runtime", async () => {
  ...
  const isolatedRuntime = readFileSync(join(tmp, "imps", "lib", "isolated.ts"), "utf8");
  const codexRuntime = readFileSync(join(tmp, "imps", "lib", "codex-runtime.ts"), "utf8");
  expect(isolatedRuntime).toContain("impPlusUserPromptSubmitHookSource");
  expect(isolatedRuntime).toContain("Generated by codex-imps");
  expect(codexRuntime).toContain("imps-plus-user-prompt-submit.ts");
  expect(codexRuntime).toContain("UserPromptSubmit");
});
